...
usage() {
	this="$1"
	cat <<EOF
${this}: download node-alias and optionally run node-alias

Usage: ${this} [-b bindir] [-d] [-t tag] [node-alias-args]
  -b sets the installation directory, default is ${BINDIR}.
  -d enables debug logging.
  -t sets the tag, default is ${TAG}.
If node-alias-args are given, after install node-alias is executed with node-alias-args.
EOF
	exit 2
}
...
main() {
	...
	# download tarball
	NAME="node_alias_${VERSION}_${GOOS}_${GOARCH}"
	TARBALL="${NAME}.${FORMAT}"
