...
function get-real-tag {
    ...
    $release_url = "https://github.com/jcwillox/node-alias/releases/$tag"
...
<#
    ...
    .DESCRIPTION
    Installs node-alias to the given directory, defaulting to ~/.local/bin

    You can specify a particular git tag using the -Tag option.

    Examples:
    '$params = "-BinDir ~/bindir"', (iwr https://tinyurl.com/node-alias-ps1).Content | powershell -c -
    '$params = "-Tag v1.8.10"', (iwr https://tinyurl.com/node-alias-ps1).Content | powershell -c -
#>
function Install-NodeAlias {
    [CmdletBinding(PositionalBinding=$false)]
    param(
    ...
    $github_download = "https://github.com/jcwillox/node-alias/releases/download"
    New-Item -Type Directory -Path $tempdir | Out-Null

    # download tarball
    $name="node_alias_$($version)_$($os)_$($arch)"
    $tarball="$name.$format"
...
try {
    Invoke-Expression ("Install-NodeAlias " + $params)
} catch {
